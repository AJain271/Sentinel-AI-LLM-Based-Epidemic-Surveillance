"""
Compare extraction methods: Zero-Shot vs Regex vs Chain-of-Thought.
...
_COMPARISON_DIR = _V2_DIR / "output_comparison"

# Method definitions: (label, output_dir)
METHODS = [
    ("Zero-Shot", _V2_DIR / "output"),
    ("Regex", _V2_DIR / "output_regex"),
    ("Chain-of-Thought", _V2_DIR / "output_cot"),
]

# ─── Styling ─────────────────────────────────────────────────────────────────

_COLORS = ["#4C72B0", "#DD8452", "#55A868"]  # blue, orange, green
plt.rcParams.update({
    "figure.dpi": 150,
    "savefig.dpi": 150,
    "font.size": 11,
...
def load_all_reports() -> List[tuple]:
    """Return list of (method_name, report_dict) for available methods."""
    loaded = []
    for name, out_dir in METHODS:
        ...
        if report is not None:
            ...
            print(f"  ⚠ No evaluation_report.json found for {name} in {out_dir}")
    return loaded
...
def print_summary(reports: List[tuple]) -> Dict[str, Any]:
    """Print a comparative summary table and return structured summary."""

    print(f"\n{'='*75}")
    print("COMPARATIVE SUMMARY")
    print(f"{'='*75}\n")

    header = f"{'Method':<20} {'Accuracy':>10} {'F1-Neg':>8} {'F1-NotP':>8} {'F1-Pres':>8} {'Novel':>8}"
    print(header)
    print("─" * 75)
    ...
    for name, report in reports:
        ...
        print(f"{name:<20} {acc:>9.1%} {f1_neg:>8.3f} {f1_notp:>8.3f} {f1_pres:>8.3f} {novel_str:>8}")
...
def plot_aggregate_accuracy(reports: List[tuple], save_dir: Path) -> None:
    """Bar chart of aggregate accuracy by method."""
    names = [n for n, _ in reports]
    accuracies = [r.get("aggregate", {}).get("accuracy", 0) for _, r in reports]

    fig, ax = plt.subplots(figsize=(7, 5))
    bars = ax.bar(names, accuracies, color=_COLORS[:len(names)], width=0.5, edgecolor="black", linewidth=0.5)
    ...
    ax.set_title("Aggregate Accuracy by Extraction Method")
...
def plot_per_class_f1(reports: List[tuple], save_dir: Path) -> None:
    """Grouped bar chart of F1 scores for negated / not_present / present."""
    ...
    ax.set_title("Per-Class F1 Score by Extraction Method")
...
def plot_novel_recall(reports: List[tuple], save_dir: Path) -> None:
    """Bar chart of novel symptom recall by method."""
    names = []
    recalls = []
    for name, report in reports:
    ...
    if not names:
        print("  ⚠ No novel symptom recall data available.")
        return

    fig, ax = plt.subplots(figsize=(7, 5))
    bars = ax.bar(names, recalls, color=_COLORS[:len(names)], width=0.5,
    ...
    ax.set_title("Novel Symptom Recall by Extraction Method")
    ax.spines[["top", "right"]].set_visible(False)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f"{y:.0%}"))

    fig.tight_layout()
    fig.savefig(save_dir / "novel_recall.png")
    plt.close(fig)
    print("  ✓ novel_recall.png")


def plot_per_transcript_accuracy(reports: List[tuple], save_dir: Path) -> None:
    """Grouped bar chart of per-transcript accuracy across methods."""
    ...
    short_names = []
    for fn in all_filenames:
        # Extract case type and patient name
        parts = fn.replace(".txt", "").split("_")
        case_type = parts[0] if parts else fn
        short_names.append(case_type + "\n" + "_".join(parts[3:5]) if len(parts) >= 5 else fn[:20])
    ...
    for i, (name, report) in enumerate(reports):
        ...
        for bar, acc in zip(bars, accs):
            if acc > 0:
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.005,
                        f"{acc:.0%}", ha="center", va="bottom", fontsize=8, rotation=45)

    ax.set_xticks(x)
    ax.set_xticklabels(short_names, fontsize=8, ha="center")
    ax.set_ylim(0, 1.12)
    ax.set_ylabel("Accuracy")
    ax.set_title("Per-Transcript Accuracy by Extraction Method")
...
def plot_confusion_matrices(reports: List[tuple], save_dir: Path) -> None:
    """Side-by-side confusion matrices for each method."""
    n = len(reports)
    ...
    fig.suptitle("Confusion Matrices by Method", fontsize=14, y=1.02)
    fig.tight_layout()
