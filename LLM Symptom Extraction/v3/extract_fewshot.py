"""
Few-shot LLM symptom extraction with quote-binding (v3).

Same task as extract_zeroshot.py but prepends three worked examples
(novel virus, flu-like, differential) so the model learns the
expected output structure — especially the unmapped/quote-binding format.

The few-shot examples use FAKE novel symptoms (not the real study
hallmarks) so no bias is introduced.
"""
...
from few_shot_examples import format_examples_block
...
def build_prompt(transcript: str) -> Tuple[str, str]:
    """Return (system_message, user_message) for the few-shot extraction call."""
...
WORKED EXAMPLE
════════════════════════════════════════════════════════════════
Study the following example carefully.  It shows the exact
input/output format expected, including how to quote-bind unmapped
symptoms.  NOTE: In the example output, only non-zero checklist
scores are shown for brevity — all other symptoms are scored 0.
Your output must include ALL {len(KNOWN_SYMPTOM_LIST)} symptoms.
...
════════════════════════════════════════════════════════════════
TRANSCRIPT (extract from this one)
════════════════════════════════════════════════════════════════
{transcript}
"""
...
def extract_single(transcript_path: Path, client: OpenAI) -> Dict[str, Any]:
    """Run few-shot extraction on a single transcript file."""
...
def extract_all(
    ...
    """Extract symptoms using few-shot prompting and save results as JSON."""
    ...
    output_dir = output_dir or (OUTPUT_DIR.parent / "output_fewshot")
    ...
    for tp in transcript_paths:
        print(f"[Few-Shot] Extracting: {tp.name} ...")
    ...
    print(f"\n[Few-Shot] Done. {len(results)} transcripts processed.")
    return results
