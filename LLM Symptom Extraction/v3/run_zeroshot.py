"""
Runner for the v3 zero-shot LLM symptom extraction pipeline.

Usage:
    python run_zeroshot.py                # extract + evaluate on dev sample
    python run_zeroshot.py --extract      # extract only
    python run_zeroshot.py --evaluate     # evaluate only (requires prior extraction)
"""
...
from extract_zeroshot import extract_all
from evaluate import evaluate_all


_OUTPUT = OUTPUT_DIR  # output/


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Zero-shot LLM symptom extraction (v3)."
    )
