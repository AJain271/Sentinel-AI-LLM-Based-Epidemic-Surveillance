...
def main():
    ...
    # Cluster scatter — legend includes ground-truth composition
    fig, ax = plt.subplots(figsize=(12, 10))
    ...
    for idx, cid in enumerate(unique_labels):
        ...
        if cid == -1:
            ...
            # Build a descriptive label from ground-truth composition
            info = per_cluster[str(cid)]
            comp = info["detail_composition"]
            parts = []
            for cat, cnt in sorted(comp.items(), key=lambda x: -x[1]):
                short = type_labels.get(cat, cat)
                parts.append(f"{short}:{cnt}")
            comp_str = ", ".join(parts)
            label = f"C{cid} ({mask.sum()}) — {comp_str}"
            ax.scatter(coords_2d[mask, 0], coords_2d[mask, 1],
                       c=[color], label=label,
                       s=50, alpha=0.8, edgecolors="white", linewidths=0.4)
    ax.set_title("HDBSCAN Clusters (UMAP 2D, EOM)", fontsize=16, fontweight="bold")
    ax.legend(loc="lower right", fontsize=14, markerscale=1.6, framealpha=0.9,
              borderpad=1.0, labelspacing=0.7, handletextpad=0.8)
    ax.set_xlabel("UMAP 1", fontsize=12); ax.set_ylabel("UMAP 2", fontsize=12)
    ax.grid(True, alpha=0.15)
    plt.tight_layout()
